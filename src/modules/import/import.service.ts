...
export class ImportService {
  ...
  async importUsersFromBuffer(buffer: Buffer) {
    ...
    for (let i = 0; i < validData.length; i++) {
      ...
      const originalRow = rawData.find(
        (row) =>
      ...
      try {
        ...
        const countryCode =
          userData.countryCode && userData.countryCode.trim() !== '' ? userData.countryCode : 'CO'; // Por defecto Colombia
        console.log(
          `Usuario: ${userData.userName}, CountryCode original: "${userData.countryCode}", CountryCode final: "${countryCode}"`,
        );
