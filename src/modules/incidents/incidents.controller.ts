...
export class IncidentsController {
  ...
  @ApiBody({ 
    schema: {
      ...
      properties: {
        ...
        description: { type: 'string', example: 'El vehículo está estacionado en lugar prohibido y obstaculiza el paso' },
        status: { type: 'string', enum: ['PENDING', 'IN_PROGRESS', 'RESOLVED', 'CANCELLED'], example: 'PENDING' },
  ...
  async createVehicleIncident(
    @Body(new ZodValidationPipe(CreateVehicleIncidentSchema)) createDto: z.infer<typeof CreateVehicleIncidentSchema>
  ) {
  ...
  @ApiQuery({ name: 'limit', required: false, description: 'Cantidad de incidentes por página', example: 10 })
  @ApiQuery({ name: 'search', required: false, description: 'Búsqueda por título, descripción o datos del incidente' })
  @ApiQuery({ name: 'type', required: false, description: 'Filtrar por tipo', enum: ['VEHICLE', 'PET', 'COEXISTENCE', 'DAMAGE'] })
  @ApiQuery({ name: 'status', required: false, description: 'Filtrar por estado', enum: ['PENDING', 'IN_PROGRESS', 'RESOLVED', 'CANCELLED'] })
  @ApiQuery({ name: 'priority', required: false, description: 'Filtrar por prioridad', enum: ['LOW', 'MEDIUM', 'HIGH', 'CRITICAL'] })
  @ApiQuery({ name: 'reportedBy', required: false, description: 'Filtrar por usuario que reportó' })
  @ApiQuery({ name: 'sortBy', required: false, description: 'Campo para ordenar', enum: ['createdAt', 'updatedAt', 'status', 'priority', 'type'] })
  ...
  @ApiResponse({ status: 200, description: 'Incidente actualizado exitosamente', type: IncidentResponseDto })
  @ApiResponse({ status: 404, description: 'Incidente no encontrado' })
  async update(
    @Param(new ZodValidationPipe(IncidentIdSchema)) params: z.infer<typeof IncidentIdSchema>,
    @Body(new ZodValidationPipe(UpdateIncidentSchema)) updateDto: z.infer<typeof UpdateIncidentSchema>
  ) {
  ...
  @ApiResponse({ status: 200, description: 'Estado del incidente actualizado exitosamente', type: IncidentResponseDto })
  @ApiResponse({ status: 404, description: 'Incidente no encontrado' })
  async changeStatus(
    @Param(new ZodValidationPipe(IncidentIdSchema)) params: z.infer<typeof IncidentIdSchema>,
