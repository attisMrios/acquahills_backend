...
export const CreateUserSchema = z.object({
  ...
  role: z.nativeEnum(UserRole).refine((val) => ['admin', 'manager', 'user'].includes(val), {
    message: 'El rol debe ser admin, manager o user',
  }),
  countryCode: z
    .string()
    .min(2, 'El código de país es requerido')
    .max(3, 'El código de país no puede exceder 3 caracteres'),
  phone: z
    .string()
    .min(5, 'El número local es requerido')
    .max(20, 'El número local no puede exceder 20 dígitos')
    .regex(/^\d+$/, 'El número local solo debe contener dígitos'),
  fullPhone: z
    .string()
    .min(5, 'El número internacional es requerido')
    .max(20, 'El número internacional no puede exceder 20 dígitos')
    .regex(/^\d+$/, 'El número internacional solo debe contener dígitos (sin +)'),
  address: z.string().optional(),
  birthDate: z.string().optional(),
  dni: z
    .string()
    .min(4, 'El DNI debe tener al menos 4 caracteres')
...
export const FilterUsersSchema = z.object({
  apartmentId: z.coerce
    .number()
    .min(1, 'El ID del apartamento debe ser un número válido mayor a 0'),
