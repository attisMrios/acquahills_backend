...
export class UnavailableDayDto {
  @ApiProperty({
    description:
      'Día de la semana. REQUERIDO si isFirstWorkingDay es false, DEBE SER NULL si isFirstWorkingDay es true',
    enum: WeekDay,
  ...
  @ApiProperty({
    description:
      'Indica si es el primer día laboral. Si es true, weekDay DEBE SER NULL. Si es false, weekDay es REQUERIDO y debe ser un valor válido del enum.',
    example: false,
    required: true,
  })
...
export class CreateCommonAreaSwaggerDto {
  ...
  @ApiProperty({
    description:
      'Días no disponibles. Cada objeto debe cumplir las reglas: si isFirstWorkingDay=true, weekDay debe ser null; si isFirstWorkingDay=false, weekDay es requerido',
  ...
  @ApiProperty({
    description:
      'Horarios disponibles. Los horarios no pueden superponerse entre sí. Formato HH:MM requerido.',
    type: [TimeSlotDto],
    example: [
      {
        startTime: '09:00',
